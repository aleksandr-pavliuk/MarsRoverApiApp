package ua.org.marsroverapiapp.service;
...
import ua.org.marsroverapiapp.repository.PreferencesRepository;
import ua.org.marsroverapiapp.response.MarsRoverApiResponse;
import ua.org.marsroverapiapp.response.MarsPhoto;
...
public class MarsRoverApiService {
  ...
  public MarsRoverApiService() {
    ...
    validCameras.put("Curiosity",
        Arrays.asList("FHAZ", "RHAZ", "MAST", "CHEMCAM", "MAHLI", "MARDI", "NAVCAM"));
  ...
  public List<String> getApiUrlEndpoints(HomeDto homeDto)
    ...
    for (Method method : methods) {
      if (method.getName().contains("getCamera") && Boolean.TRUE.equals(
          method.invoke(homeDto))) {
        ...
        if (validCameras.get(homeDto.getMarsApiRoverData()).contains(cameraName)) {
          urls.add("https://api.nasa.gov/mars-photos/api/v1/rovers/" + homeDto.getMarsApiRoverData()
              + "/photos?sol=" + homeDto.getMarsSol() + "&api_key=" + API_KEY + "&camera="
              + cameraName);
        }
      }
    }
